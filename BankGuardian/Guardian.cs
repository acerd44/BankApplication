...
namespace BankGuardian
{
    public class Guardian
    {
        ...
        public void Check()
        {
            ...
            int transactionsChecked = alreadyCheckedTransactions.Count;
            for (int countryIndex = 0; countryIndex < 4; countryIndex++)
            {
                ...
                transactions = _transactionService.GetTransactions(countries[countryIndex])
                    .Where(t => !alreadyCheckedTransactions.Contains(t.TransactionId)).ToList();
                ...
                if (alreadyCheckedTransactions.Count > transactionsChecked)
                {
                    using (StreamWriter sw3 = new("../../../reports/checked_transactions.txt", true))
                    {
                        alreadyCheckedTransactions.ForEach(t => sw3.WriteLine(t.ToString()));
                    }
                }
            ...
            Console.WriteLine($"\nSuspicious customers found: {customersAboveLimit.Count}");
            Console.WriteLine("_____________________________________________________________________");
            using (StreamWriter sw = new($"../../../reports/suspicious_customers_{DateTime.Now.AddDays(-3):yyyyMMdd}-{DateTime.Now:yyyyMMdd}.txt"))
            {
                ...
                {
                    using (StreamWriter sw2 = new($"../../../reports/checked_customers_{DateTime.Now.AddDays(-3):yyyyMMdd}-{DateTime.Now:yyyyMMdd}.txt", true))
                    {
                        sw2.WriteLine(c);
                    }
